...
def character_name: stem | split("-")[0];
def n: tostring;
...
def character_initializer:
  .file as $file
  | .data as $data
  | [
      "    Character{",
      "        \"" + ($file | filename | stem | character_name) + "\",",
      "        {",
...
| [
    ...
    "namespace",
    "{",
    "const std::array<Character, " + ($sources | length | tostring) + "> characters {{",
    ($sources[] | character_initializer),
    "}};",
    "}",
    "",
    "std::span<const Character> get_characters()",
    "{",
    "    return characters;",
    "}"
  ] | join("\n")
