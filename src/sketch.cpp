...
const std::vector<Point> &Sketch::get_points() const { return points_; }

const int &Sketch::get_zoom() const { return zoom_; }
...
std::vector<Point> Sketch::simplify() const {
  ...
  for (auto it = points_.begin();;) {
    double old_angle;
    if (points.size() >= 3) {
      old_angle = get_angle(*points.rbegin(), *(points.rbegin() + 1),
                            *(points.rbegin() + 2));
      if (std::abs(old_angle) > angle_max ||
          get_distance(*(points.rbegin() + 1), *(points.rbegin() + 2)) <
              tolerance) {
    ...
    if (it == points_.end()) {
      if (points.size() >= 2 &&
          get_distance(*points.rbegin(), *(points.rbegin() + 1)) < tolerance) {
        *(points.rbegin() + 1) = *(points.rbegin());
        points.pop_back();
      }
      break;
    ...
    double new_angle = get_angle(*it, *points.rbegin(), *(points.rbegin() + 1));
    ...
    if (old_angle * new_angle < 0.0) {
      ...
      auto n1 =
          get_nearst(*(points.rbegin() + 1), {*it, *(points.rbegin() + 2)});
      if (get_distance(n1, *(points.rbegin() + 1)) > tolerance / 2.0 ||
          get_distance(n1, mid) > length + tolerance) {
        points.push_back(*it++);
        continue;
      }
      auto n2 = get_nearst(*points.rbegin(), {*it, *(points.rbegin() + 2)});
...
Shape::Type Sketch::get_type() const { return Type::SKETCH; }
